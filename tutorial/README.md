...
SVM-MOCCA and RF-MOCCA distinguish themselves from classical use of SVM and RF with motifs, where SVMs or RFs are trained with motif occurrence frequencies or *k*-spectra, and the MOCCA methods train one model per motif and combine predictions.
As MOCCA models are the main novelty of the MOCCA suite, we will focus on the training of these models first.
